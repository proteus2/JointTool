function [k1,fl,re,Ke,varargout] = locel(u,tl1,tl2,L,C1,C2,varargin)
...
Ke = Kl(1:12,1:12) + Kl(1:12,13:20)*A;

if twist_flag == 1
    qe = [0;0;0;tl1;u;0;0;tl2-[morph.twist;0;0]];
    dqedphi        = zeros(size(qe));
    dqedphi(4:6)   = [0;0;0];
    dqedphi(10:12) = [-1;0;0];
    dredphi        = Ke*dqedphi;
    dqedphi        = dqedphi([7 4 5 6 10 11 12]);
    dfldphi        = dredphi([7 4 5 6 10 11 12]);
    morpho.dqedphi  = dqedphi;
    morpho.dfldphi  = dfldphi;
    morpho.dredphi = dredphi;
    if isfield(morph,'dptl1')
        dpqe          = zeros(12,size(morph.dptl1,2));
        dpqe(4:6,:)   = morph.dptl1;
        dpqe(7,:)     = morph.dpu;
        dpqe(10:12,:) = morph.dptl2;
        dpre          = Ke*dpqe;
        dpfl          = dpre([7 4 5 6 10 11 12],:);
        morpho.dpfl    = dpfl;
    end
    varargout{1} = morpho;
elseif twist_flag == 0
    qe = [0;0;0;tl1;u;0;0;tl2];
end

if shear_flag == 1 && isfield(morph,'dpsitl1')
    dpsiqe  = zeros(12,size(morph.dpsitl1,2));
    dpsiqe(4:6,:) = morph.dpsitl1;
    dpsiqe(10:12,:) = morph.dpsitl2;
    dpsire  = Ke*dpsiqe;
    dpsifl  = dpsire([7 4 5 6 10 11 12],:);
    dpsiqe  = dpsiqe([7 4 5 6 10 11 12],:);
    morpho.dfldpsi = dpsifl;
    morpho.dqedpsi = dpsiqe;
    morpho.dredpsi = dpsire;
    if isfield(morph,'d2psitl1')
        d2psiqe = zeros(size(qe));
        d2psiqe(4:6,:) = morph.d2psitl1;
        d2psiqe(10:12,:) = morph.d2psitl2;
        d2psiqe = d2psiqe([7 4 5 6 10 11 12],:);
        morpho.d2qedpsi = d2psiqe;
    end
    if isfield(morph,'dpsidptl1')
        dpsidpqe = zeros(size(qe,1),12);
        dpsidpqe(4:6,:) = morph.dpsidptl1;
        dpsidpqe(10:12,:) = morph.dpsidptl2;
        dpsidpqe = dpsidpqe([7 4 5 6 10 11 12],:);
        morpho.dqedpsidp = dpsidpqe;
...
if fold_flag == 1 && isfield(morph,'dthetatl1')
    dthetaqe  = zeros(12,size(morph.dthetatl1,2));
    dthetaqe(4:6,:) = morph.dthetatl1;
    dthetaqe(10:12,:) = morph.dthetatl2;
    dthetare  = Ke*dthetaqe;
    dthetafl  = dthetare([7 4 5 6 10 11 12],:);
    dthetaqe  = dthetaqe([7 4 5 6 10 11 12],:);
    morpho.dfldtheta = dthetafl;
    morpho.dqedtheta = dthetaqe;
    morpho.dredtheta = dthetare;
    if isfield(morph,'d2thetatl1')
        d2thetaqe = zeros(size(qe));
        d2thetaqe(4:6,:) = morph.d2thetatl1;
        d2thetaqe(10:12,:) = morph.d2thetatl2;
        d2thetaqe = d2thetaqe([7 4 5 6 10 11 12],:);
        morpho.d2qedtheta = d2thetaqe;
    end
    if isfield(morph,'dthetadptl1')
        dthetadpqe = zeros(size(qe,1),12);
        dthetadpqe(4:6,:) = morph.dthetadptl1;
        dthetadpqe(10:12,:) = morph.dthetadptl2;
        dthetadpqe = dthetadpqe([7 4 5 6 10 11 12],:);
        morpho.dqedthetadp = dthetadpqe;
    ...
    if shear_flag == 1 && isfield(morph,'dpsidthetatl1')
        dpsidthetaqe = zeros(size(qe,1),1);
        dpsidthetaqe(4:6,:) = morph.dpsidthetatl1;
        dpsidthetaqe(10:12,:) = morph.dpsidthetatl2;
        dpsidthetaqe = dpsidthetaqe([7 4 5 6 10 11 12],:);
        morpho.dqedpsidtheta = dpsidthetaqe;
    end
    varargout{1} = morpho;
end
...
re = Ke*qe;
k1 = Ke([7 4 5 6 10 11 12],[7 4 5 6 10 11 12]);
fl = re([7 4 5 6 10 11 12]);
